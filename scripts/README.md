...
parameter parity, risk/planning and retry/idempotency invariants, deterministic
parameter/result `SchemaId` values, stable per-operation diagnostic vocabularies,
evidence and capability requirements, and a classification SHA-256. The Rust
...
Generates 59 closed Draft 2020-12 parameter schemas from the canonical registry
and one complete projection table for all 33 stable diagnostic codes. Canonical
