...
/// Runtime status strings persisted in `ai_hook_events.status` that count as
/// a hook failure anchor. Storage and scanner adapters map their status types
/// into these stable domain values before evaluation.
