...
fn recency_decays_monotonically_to_floor() {
    ...
    assert!((now - 1.0).abs() < 1e-9, "delta_t=0 -> full recency");
    // At one half-life the (1-phi) component halves: phi + (1-phi)*0.5.
...
fn never_decay_reason_keeps_full_confidence() {
    // ai_session_project (lambda=0) does not decay even after a long gap.
...
fn trust_ceiling_caps_correlated_and_zeroes_refuted() {
    ...
    assert!((apply_trust_ceiling(0.9, "verified") - 0.9).abs() < 1e-9);
