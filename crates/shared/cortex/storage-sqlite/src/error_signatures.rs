...
use anyhow::Result;
use cortex_domain::ErrorSignatureEntry;
use rusqlite::params;
...
pub fn cursor_get(pool: &DbPool) -> Result<i64> {
    let conn = pool.get()?;
...
pub fn cursor_advance(conn: &rusqlite::Connection, new_last_id: i64) -> Result<()> {
    conn.execute(
...
pub struct UpsertSignatureParams<'a> {
    pub hash: &'a str,
...
pub fn upsert_signature(conn: &rusqlite::Connection, p: UpsertSignatureParams<'_>) -> Result<()> {
    conn.execute(
...
pub fn read_unaddressed_page(
    pool: &DbPool,
    limit: i64,
    offset: i64,
    include_acknowledged: bool,
) -> Result<Vec<ErrorSignatureEntry>> {
    let conn = pool.get()?;
...
pub fn read_signature_by_hash(
    pool: &DbPool,
    signature_hash: &str,
    normalizer_version: i64,
) -> Result<Option<ErrorSignatureEntry>> {
    let conn = pool.get()?;
    let cutoff_1h = chrono::Utc::now()
        .checked_sub_signed(chrono::TimeDelta::hours(1))
        .map(|dt| dt.format("%Y-%m-%dT%H:%M:%S%.3fZ").to_string())
