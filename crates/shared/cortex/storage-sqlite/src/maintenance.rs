...
pub fn db_incremental_vacuum(pool: &DbPool, pages: u32) -> Result<()> {
    let conn = pool.get()?;
    let _write_guard = crate::write_lock();
...
/// Trusted SQLite PRAGMA names exposed by the storage diagnostics port.
///
/// An enum keeps SQL identifier interpolation closed to a fixed allow-list.
...
impl PragmaName {
    const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::FreelistCount => "freelist_count",
            Self::PageSize => "page_size",
            Self::AutoVacuum => "auto_vacuum",
            Self::JournalMode => "journal_mode",
            Self::CacheSize => "cache_size",
            Self::MmapSize => "mmap_size",
        }
    }
}

/// Read one trusted integer-valued SQLite PRAGMA.
pub fn db_pragma_i64(pool: &DbPool, pragma: PragmaName) -> Result<i64> {
    let conn = pool.get()?;
    Ok(conn.query_row(&format!("PRAGMA {}", pragma.as_str()), [], |row| row.get(0))?)
}

/// Read one trusted string-valued SQLite PRAGMA.
pub fn db_pragma_string(pool: &DbPool, pragma: PragmaName) -> Result<String> {
    let conn = pool.get()?;
    Ok(conn.query_row(&format!("PRAGMA {}", pragma.as_str()), [], |row| row.get(0))?)
}
...
///
/// Uses the private `fts_incremental_merge` helper after the loop because FTS5 DELETE triggers
