...
use crate::{DbPool, LogBatchEntry, init_pool, insert_logs_batch, list_hosts, tail_logs};
use anyhow::Result;
use rusqlite::params;
use std::path::Path;
...
fn external_disk_pressure_does_not_delete() {
    ...
    // Notification policy consumes this storage outcome in the application layer.
    // This crate proves only the persistence invariant: external pressure blocks
    // writes without deleting retained data.
}
