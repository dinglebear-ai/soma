...
//! `crate::inputs`; this module only persists and reads back
...
use crate::inputs::ExtractedMcpEvent;
pub use cortex_domain::McpEventEntry as AiMcpEventEntry;
...
pub(crate) fn insert_mcp_events_in_tx(
    ...
    for item in events {
        let (tool_name, mcp_server, mcp_tool) = if item.event.event_kind
            == crate::inputs::McpEventKind::Result
        ...
        let changed = tx.prepare_cached(
            ...
            (item.event.event_kind == crate::inputs::McpEventKind::Call)
                .then_some(item.log_id),
            (item.event.event_kind == crate::inputs::McpEventKind::Result)
                .then_some(item.log_id),
            item.ai_tool,
            item.ai_project,
            item.ai_session_id,
