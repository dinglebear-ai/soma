...
fn test_pool() -> (DbPool, tempfile::TempDir) {
    ...
    let pool = crate::init_pool(&storage).unwrap();
    (pool, dir)
}
...
fn seed_inventory(pool: &DbPool, services: usize) {
    ...
    let _guard = crate::graph::GRAPH_TEST_LOCK.lock();
    crate::graph::refresh_graph_projection(pool).unwrap();
    crate::graph_inventory::project_inventory(pool, &inventory).unwrap();
}
...
fn mount_findings_are_bounded_and_use_relationship_type_index() {
    ...
    let plan = relationship_type_query_plan(&pool, crate::graph::REL_MOUNTS).unwrap();
