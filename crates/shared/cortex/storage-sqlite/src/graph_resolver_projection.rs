...
use crate::graph::{self, EntityMemo, EvidenceInput, GraphWalkEntity, LogGraphRow};
...
/// exhaustive one (mirrors [`crate::graph::graph_around_entity`]'s
...
pub(crate) fn extract_agent_docker_row(
    ...
    let decisions = crate::entity_resolution::resolve_observations(&observations);
...
fn agent_docker_observations_from_log_row(
    row: &LogGraphRow,
    meta: Option<&Value>,
) -> Vec<crate::entity_resolution::ResolverObservation> {
    ...
    let identity = crate::entity_resolution::AgentDockerIdentity {
        agent_host,
        container_id,
        container_name,
    ...
    crate::entity_resolution::observations_from_agent_docker_identity(&identity)
}
...
fn project_resolver_decisions(
    conn: &rusqlite::Connection,
    row: &LogGraphRow,
    decisions: &[crate::entity_resolution::ResolvedEntityDecision],
    memo: &mut EntityMemo,
) -> Result<()> {
    ...
    for (instance_key, instance_id) in instance_ids {
        if let Some((_, service)) =
            crate::entity_resolution::split_service_instance_key(&instance_key)
...
pub(crate) fn trust_to_graph(trust: crate::entity_resolution::ResolverTrust) -> &'static str {
    match trust {
        crate::entity_resolution::ResolverTrust::Verified => graph::TRUST_VERIFIED,
        crate::entity_resolution::ResolverTrust::Claimed => graph::TRUST_CLAIMED,
        crate::entity_resolution::ResolverTrust::Inferred => graph::TRUST_INFERRED,
    }
}
