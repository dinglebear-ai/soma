...
use cortex_ingest_core::SourceKind;
...
fn lint_fts_query(query: &str) -> Result<()> {
    ...
    if has_unquoted_hyphen {
        return Err(anyhow::Error::new(
            cortex_domain::DomainError::InvalidInput(
    ...
    if !query.matches('"').count().is_multiple_of(2) {
        return Err(anyhow::Error::new(
            cortex_domain::DomainError::InvalidInput(
                "unbalanced quote in search query; wrap phrases in matching double quotes"
                    .to_string(),
            ),
        ));
    }
...
pub fn validate_fts_query(query: &str) -> Result<()> {
    ...
    if query.len() > 512 {
        return Err(anyhow::Error::new(
            cortex_domain::DomainError::InvalidInput(format!(
...
pub fn page_agent_projection_logs(
    pool: &DbPool,
    after_id: i64,
    limit: usize,
) -> Result<Vec<LogEntry>> {
...
/// the private `ai_rows_watermark` fingerprint.
///
...
///   the `WalkReached` host-fanout mode)
