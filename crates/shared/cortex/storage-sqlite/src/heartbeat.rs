...
pub use cortex_domain::{
    HeartbeatHostState, HeartbeatSampleState, HeartbeatStateFlags, HeartbeatWindowSummary,
};
...
/// Derive canonical domain heartbeat flags from a fully-loaded storage sample.
pub(crate) fn heartbeat_flags(sample: &HeartbeatSampleState) -> HeartbeatStateFlags {
    cortex_domain::heartbeat_flags_from_sample(sample)
}
