//! SQLite persistence adapter extracted from Cortex.
//!
//! The adapter owns schema/migration state, connection management, transactional
//! writes, query projections, graph and observatory persistence, and bounded
//! storage maintenance. Semantic contracts come from `cortex-domain`; ingest
//! normalization comes from `cortex-ingest-core`.

#[cfg(test)]
#[path = "agent_observatory_tests.rs"]
mod agent_observatory_tests;
...
mod pool;
mod queries;
pub use queries::page_agent_projection_logs;
mod queries_hosts;
...
pub use inputs::{
    ExtractedHookEvent, ExtractedMcpEvent, ExtractedSkillEvent, HookEvidenceKind, HookStatus,
    McpEventKind, SkillEventKind, SkillEvidenceKind,
};

pub use analytics::PATTERN_SCAN_LIMIT_MAX;
pub use analytics::{
    ...
    feed_logs, fetch_log_by_id, fetch_patterns, get_ai_project_context, get_ai_usage_blocks,
    ingest_rate, ingest_rate_by_host, list_apps, list_source_ips, silent_hosts, summarize_range,
    timeline,
};
pub use graph::{
...
pub use maintenance::{
    DiskSpaceProbe, MaintenanceJob, SystemDiskSpaceProbe, checkpoint_wal_and_incremental_vacuum,
    ...
    maybe_checkpoint_wal_by_size, physical_size_bytes, purge_by_tag_window, purge_old_heartbeats,
    purge_old_llm_invocations, purge_old_logs, wal_checkpoint_complete,
};
pub use maintenance::{PragmaName, db_pragma_i64, db_pragma_string};
pub use mcp_events::{
