...
fn run_wasm(
    ...
    let input_alloc = typed::<i32, i32>(&instance, &mut store, "rtemplate_input_alloc")?;
    let input_ptr_fn = typed::<(), i32>(&instance, &mut store, "rtemplate_input_ptr")?;
    let call_fn = typed::<(), i32>(&instance, &mut store, "rtemplate_call")?;
    let output_ptr_fn = typed::<(), i32>(&instance, &mut store, "rtemplate_output_ptr")?;
    let output_len_fn = typed::<(), i32>(&instance, &mut store, "rtemplate_output_len")?;
