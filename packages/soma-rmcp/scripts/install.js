...
const { binaryPath, checksumUrl, downloadUrl, releaseVersion, targetFor } = require("../lib/platform");
...
function log(message) {
  process.stderr.write(`soma-rmcp: ${message}\n`);
}
...
function verifyChecksum(archive, checksumFile, asset) {
...
function verifyAttestation(archive, repo, version, runner = spawnSync) {
...
async function main() {
  if (process.env.SOMA_RMCP_SKIP_DOWNLOAD === "1") {
    log("skipping binary download because SOMA_RMCP_SKIP_DOWNLOAD=1");
    return;
  }
  ...
  requireGhVersion();
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "soma-rmcp-install-"));
  const archive = path.join(tempDir, target.asset);
  const checksums = path.join(tempDir, "SHA256SUMS");
  try {
    const options = {
      timeoutMs: parseControl(process.env.SOMA_RMCP_DOWNLOAD_TIMEOUT_MS, DEFAULT_TIMEOUT_MS, "SOMA_RMCP_DOWNLOAD_TIMEOUT_MS", { minimum: 1 }),
      connectTimeoutMs: parseControl(process.env.SOMA_RMCP_CONNECT_TIMEOUT_MS, DEFAULT_CONNECT_TIMEOUT_MS, "SOMA_RMCP_CONNECT_TIMEOUT_MS", { minimum: 1 }),
      maxRedirects: parseControl(process.env.SOMA_RMCP_MAX_REDIRECTS, DEFAULT_MAX_REDIRECTS, "SOMA_RMCP_MAX_REDIRECTS", { minimum: 0 }),
    };
    const url = downloadUrl(target);
    log(`downloading ${url}`);
    await download(url, archive, options);
    await download(checksumUrl(), checksums, options);
    verifyChecksum(archive, checksums, target.asset);
    const repo = process.env.SOMA_RMCP_REPO || "dinglebear-ai/soma";
    verifyAttestation(archive, repo, releaseVersion());
