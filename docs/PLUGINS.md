...
- declares the `skills` path (Soma bundles no `mcpServers` and no `hooks`)
...
- Where a server does wire lifecycle hooks, its Claude hook config calls
  `<binary> setup plugin-hook` directly (Soma itself ships no hooks — its test
  asserts no manifest declares a `hooks` key and no `hooks/hooks.json` exists)
