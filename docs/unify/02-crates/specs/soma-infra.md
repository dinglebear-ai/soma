...
13. Build contexts reject symlinks, special files, root escapes, and configured file or byte ceiling violations.
14. Build plans bind the exact source-context SHA-256 and output image artifact set.
15. Build execution re-fingerprints contexts before send and verifies every output tag afterward.
16. Container recreate plans bind the exact replacement configuration digest and image-pull choice.
17. Container recreate rechecks the digest immediately before removal and records the furthest completed destructive stage.
18. Compose recreate plans bind normalized configuration and service pre-state, then verify the exact healthy post-state.
19. Arbitrary command execution, file transfer, image deletion, pruning, and Compose down remain outside this slice.
