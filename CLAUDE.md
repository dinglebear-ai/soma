...
| Workspace | 42 cargo members (largest in the fleet) under `crates/soma/*`, `crates/shared/*`, `crates/integrations/*`, `apps/soma`, `packages/python`, plus `xtask`. Note `apps/web` and `apps/palette` are **not** cargo members — they are frontend assets. |
...
### Workspace edition and lints

The root `Cargo.toml` sets `edition = "2024"` in `[workspace.package]`; all 40
members inherit it. Workspace lints enforce the fleet `mod_module_files =
"deny"` convention together with crate-level documentation and unsafe-code
guards. Generated servers inherit this policy, so keep new members on
...
|---|---|---|---|---|
| `service.greet(name)` | `soma(action="greet")` | `soma greet [--name NAME]` | `POST /v1/greet` |  |
...
| `service.python_environment_status()` | `soma(action="python_environment_status")` | `soma python_environment_status` | `GET /v1/python/environments` |  |
| `service.python_environment_prune_plan()` | `soma(action="python_environment_prune_plan")` | `soma python_environment_prune_plan --json '{\` | `POST /v1/python/environments/prune-plan` |  |
| `service.python_environment_prune()` | `soma(action="python_environment_prune")` | `soma python_environment_prune --json '{\` | `POST /v1/python/environments/prune` |  |
| `service.python_environment_repair()` | `soma(action="python_environment_repair")` | `soma python_environment_repair --json '{\` | `POST /v1/python/environments/repair` |  |
| `service.python_environment_update()` | `soma(action="python_environment_update")` | `soma python_environment_update --json '{\` | `POST /v1/python/environments/update` |  |
| `service.python_worker_status()` | `soma(action="python_worker_status")` | `soma python_worker_status` | `GET /v1/python/workers` |  |
| `service.python_worker_cancel()` | `soma(action="python_worker_cancel")` | `soma python_worker_cancel --json '{\` | `POST /v1/python/workers/cancel` |  |
| `service.python_worker_reset()` | `soma(action="python_worker_reset")` | `soma python_worker_reset --json '{\` | `POST /v1/python/workers/reset` |  |
| `service.python_generation_status()` | `soma(action="python_generation_status")` | `soma python_generation_status` | `GET /v1/python/generations` |  |
| `service.python_generation_rollback()` | `soma(action="python_generation_rollback")` | `soma python_generation_rollback --json '{\` | `POST /v1/python/generations/rollback` |  |
| `service.python_graduation_status()` | `soma(action="python_graduation_status")` | `soma python_graduation_status --json '{\` | `POST /v1/python/graduation/status` |  |
| `service.python_graduation_apply()` | `soma(action="python_graduation_apply")` | `soma python_graduation_apply --json '{\` | `POST /v1/python/graduation/apply` |  |
...
- **PR CI uses the self-hosted Rust runner pool; native artifacts are release-only** — Linux Rust jobs use `ci-pool-rust`, and `ci.yml` plus `msrv.yml` route jobs through `cargo xtask changed-paths`. PR CI currently has no native Windows job; published releases build native artifacts on GitHub-hosted runners. Soma is public, so live branch protection is API-visible. Keep strict branch protection configured with `Repository Contract`, `CI Gate`, and `MSRV Gate`; do not require individual path-skipped jobs. Full setup and troubleshooting are in [`docs/CI.md`](docs/CI.md), [`docs/LINUX-RUNNER.md`](docs/LINUX-RUNNER.md), and [`docs/WINDOWS-RUNNER.md`](docs/WINDOWS-RUNNER.md).
